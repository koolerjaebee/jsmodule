...
3.  Verify key is valid (test in terminal:
...
**Solution**: This is now fixed in the latest version. Update jsmodule
package.
...
✅ Good: “perform linear regression with wt.loss as outcome and age,
sex, ph.ecog as predictors”
...
1.  `DEPLOYMENT_ENV` environment variable (`production` or
    `development`)
...
- ⚠️ **macOS/Windows**: Falls back to standard eval with warning in
  production mode
...
- All code is publicly available and auditable at
...
- ⚠️ **NOT RECOMMENDED**: Do not deploy with `show_api_config = TRUE`
  publicly
